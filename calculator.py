...
def get_digit(digit):
    current = result_label['text'] # to fetch what is in the screen
    new = current + str(digit) #btn on the screen + digit got from function
...
def get_operator(op):
    ...
    first_number = int(result_label['text'])  #take the result label text convert it into int and store in first
    operator = op   #take the operator
    result_label.config(text='')    #clear the screen

def get_result():
    # ab jo we've on screen is 2nd number
    global second_number, operator
...
root.resizable(0,0)   # x aur y direction me 0 movement
root.configure(background='black')

# answer display k liye
result_label = Label(root,text='',bg='black',fg='white',anchor='e')
result_label.grid(row=0,column=0,columnspan=20,pady=(50,25),sticky='w') # w=start from west, colspan will stop from destroying the grid
